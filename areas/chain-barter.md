...
## The back panel — the same history, in paint and paper

The plinth sits built into the truck itself, on side 1 of the rear
box — opposite side 2, the BosaTAZ QR codes panel, not the chalkboard.
The truck's four sides each carry one function: rear (the lavagna and
bar), front (Panem et Circenses' backdrop), side 1 (this plinth), side
2 (BosaTAZ's codes) — rear and front opposite each other, side 1 and
side 2 opposite each other. Behind the plinth, a permanent back panel:
a 10×10 grid, painted in black lines only, numbered in one corner of
each square in a snaking path — starting bottom-left, running right,
then reversing back leftward on the row above, the same boustrophedon
path as a snakes-and-ladders board, carrying none of its randomness or
competition. Just the shape of a journey, nothing to win or lose.

Each square holds a small printed photo — pulled from the Chain tab's
own digital record, printed, and glued into the next numbered square
with the same hot glue gun the Transformation Station already uses. One
photo, one trade, in order. Updating the panel is folded into Il
Custode's existing task, not a new duty invented alongside it: the same
visit that returns the object to the plinth before the next rehearsal is
when the previous trade's photo gets printed and added — no ceremony,
the same continuity already asked of the role.

The rear and side 1 run on opposite clocks: the lavagna records
promises in chalk, wiped and re-photographed every rehearsal,
ephemeral by design. The back panel records the chain's history in
paint and paper, permanent, accumulating slowly across years. One side
forgets on purpose. The other doesn't.

**When the grid fills.** A hundred squares is a real, calculable
capacity, not an abstract one. Once every square holds a photo, the
panel is retired and a fresh, blank grid takes its place. The completed
panel doesn't get discarded — it becomes its own slowly compiled
artifact, on the same footing as the Remembrancer's own annually bound
chronicle, just resolved on the chain's own pace rather than a calendar
year: however many rehearsals it takes to fill a hundred squares is
however long that particular volume took. Where the chronicle is
compiled and bound by the Remembrancer, a completed chain panel is
compiled by Il Custode — the same role, carrying the same kind of
long, patient task the Remembrancer already carries for the written
record, just for the object's own history instead of the day's. Where a
completed panel ends up afterward — displayed, stored, or something
else — isn't yet decided, the same way several of the Remembrancer's own
long-task details were left open until they mattered.
