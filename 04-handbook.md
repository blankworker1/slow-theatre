...
Alongside the documents, the origin repository has a **Discussions** board —
a public noticeboard attached to the shared repository, open to anyone,
readable by anyone, no account needed to read it and only a free GitHub
account needed to post.

This is not a replacement for a Node's own private logistics — a WhatsApp
group, a phone call, whoever's actually deciding when the truck goes out.
Discussions is the layer above that: the place where Nodes that don't yet
know each other can.

Four categories, each doing a different job:

- **Announcements** — a new Node forking for the first time; a change to the
  shared protocol.
- **Show and Tell** — a Node posting what happened at their last performance,
  linking their own remembrancer-log entries. This is where the archive gets
  a living front door, instead of just being a folder of dated files nobody
  outside the Node ever opens.
- **Protocol Proposals** — before anyone changes the shared protocol, they
  raise it here first. If it holds up, it becomes an actual proposed change
  (a "pull request" — a formal suggestion to alter the shared documents,
  which any Node, including this one, can look at, discuss, and decide
  whether to adopt).
- **Q&A** — a new Node's practical questions, answered by whoever has already
  done it.

No Node is required to participate in Discussions to exist. A Node that never
posts a word is exactly as real as one that posts every week. Discussions is
an offer, not an obligation — the same as everything else in this Handbook.
