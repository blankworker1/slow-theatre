...
plinth is the result, seams left visible on purpose — the Visual
...
ceremony itself is the reaction. A sealed plate is the result,
irreversible the way most real transmutations are — nobody runs a key
...
structured document, walking away unconsumed and reusable for the next
one, and — this is the part worth being honest about — never itself
appearing in the result. What ends up on the page was always the
...
lowering how much effort it takes to get from an unwanted object, or a
handful of raw material, to something finished.

## The part every honest version of this has to include: byproducts

Not every precursor becomes a result. Some of it is simply what didn't
transmute — offcuts, false starts, material with nothing usable left
in it. This project already has a place for exactly that, arrived at
before the chemistry language existed to name it: whatever's recycled
as waste at L'Usato, deliberately given no dedicated role, no ceremony,
ordinary disposal, not part of the performance or its economy at all.
A byproduct isn't a failure of the reaction. It's simply the part of
the precursor that was never going to become the result, however
carefully the reaction was run.

## What it isn't

A catalyst that starts claiming to be the precursor has stopped being
a catalyst and started being something this project has already named
and warned against elsewhere — the same recognition-seeking risk
attached to RESERVATO, the chain's benefactor move, and Il Suonatore:
present, useful, unconsumed, and not the reason the result is actually
worth anything. The whole value of a catalyst is that it disappears
from the finished thing cleanly, leaving no residue and claiming no
credit. The moment it stops disappearing, it isn't catalysing anymore.
It's just another ingredient, pretending it was never added.
