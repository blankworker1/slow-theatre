...
2. **Ledger / chat** — named, signed promises, posted to the relay
   ...
   visual marker for a posted promise (forward-looking, executory —
   a commitment not yet discharged). Scroll emoji (📜) as the marker
   for a *declaration* — a statement of something already true, not
   a commitment: the custody line, and any chain-object trade record.
   The two markers are deliberately opposite in register: 🤝 signals
   an obligation still pending; 📜 signals a fact already settled.
   Not specific to the chain object — any future declaration-type
   entry in the ledger uses 📜, the same way any future promise uses
   🤝.
...
5. **Archive site** — static, retrospective, not live. Receives
   ...
   anything back down.
6. **Chain tab** — append-only, local, and never reset — the one
   surface that deliberately keeps accumulating rather than
   clearing between rehearsals. Records the chain object's history:
   each form it's taken, who traded for it, what was given, and
   when — each entry marked 📜, since a trade record states a fact
   already settled, not a pending commitment. Closing entry on any
   day it was traded is the custody line, also marked 📜, naming
   whoever holds it until the next Prova (see roles/il-custode.md).
   Does not sync to BosaNode the same way the archive site does —
   this history is local to the Node by design.
7. **Promise signal light bank** — five LEDs, GPIO-driven, mounted
   above the physical blackboard. The server watches its own event
   stream and lights one bulb per pending promise, capped at five
   (beyond that, "several" rather than an exact count — subitizing
   limit, chosen so the count reads at a glance, not by counting).
   No phone or app needed to notice a new promise has arrived — the
   light is a fixed physical signal at the point of action, not a
   notification competing for attention on a screen. The
   Remembrancer clears them at their own pace: chalk one, send a
   decrement signal from their own phone over the local network,
   one bulb goes dark. This decrement channel is low-stakes and
   does NOT need the PIN-gated admin treatment — worst case of
   misuse is a light going out early, not a security exposure.
...
- Whether/how cross-node syncing works when two TAZs are
  co-located
- Final visual/UI treatment beyond the handshake (🤝) and scroll
  (📜) emoji conventions
- Remote node enclosure's final physical form (A-frame candidate
  proposed, not settled)

## Captured for later: the document footer / logos

A three-line mantra, discovered through conversation rather than designed
outright, in Italian only (the wordplay doesn't survive translation):

*La Prova*
*Teatro senza orologio*
*La Prova è una prova*

Never explained anywhere it appears. Intended uses, both deferred:

- **A standard footer across all twelve core documents** (both languages —
  the footer itself stays Italian even on English pages, the same way
  06-script-la-prova.md's own closing line does), applied at BosaNode's
  actual public launch. Not added now, since the documents are still under
  active review and not yet final.
- **The Program tab**, where it would sit naturally alongside the tab's
  own no-timer, no-countdown rule — the mantra quietly describing that
  same design principle back to whoever's looking, rather than announcing
  it.

## Captured for later: the BosaNode roadmap diagram

A two-axis interactive diagram for the BosaNode website, replacing a flat
document list with something that shows real progress rather than
implying completeness.

**Forward axis (→).** A horizontal timeline of stages — Foundation, Lo
Spuntino, 1+1, Full La Prova, BosaTAZ — spaced irregularly, not evenly,
since real rehearsal cadence isn't regular. A "here" marker sits at the
current position (before Lo Spuntino has run on its own). Each stage is
clickable, opening a short info popup — what that stage is, nothing more.
Popups never link to depth content; that's the other axis's job, kept
deliberately separate. A dashed line trails off past the last stage,
signalling the trajectory continues with no fixed floor or ceiling.

**Depth axis (↓).** Three horizontal bars — Philosophy, Script and
Staging, Legal basis — each filled to reflect how much genuinely current
content exists in that category right now, with the unfilled portion
shown dashed rather than hidden. Cumulative: as the forward axis advances,
each bar's fill grows independently, at its own pace.

**Both axes move only by manual edit, never automatically.** There's no
live tracking system behind this — moving "here," or extending a depth
bar, is a deliberate act after a real milestone is actually reached, the
same person-triggered discipline as the Remembrancer deciding when to
ring the bell. Nothing here should ever feel automated, even where it
technically could be.

**The one caption the page carries, in status-report register rather
than instructional register** — stating the two axes and the current
state in one line, so a reader leaves informed rather than just oriented:

*"BosaNode today: the philosophy is settled. The first rehearsal — Lo
Spuntino, on its own — hasn't happened yet. This roadmap only moves when
something real does: → tracks actual rehearsals run, ↓ tracks how much of
what's already real can be explored. Both update by hand, after the fact,
never in advance."*

**Explicitly not a violation of BosaTAZ's no-landing-page rule.** That
rule governs the live, in-person performance — nobody explaining what's
happening to someone standing in front of the truck. A website's own
navigation legend is a different register entirely, closer to a map's key
than a captive portal; a map that doesn't explain its own symbols isn't
more honest for it, just harder to read.
