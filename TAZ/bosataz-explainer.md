...
TAZ and ZAT are two distinct, related terms, not one acronym doing double duty. **TAZ** stays the name of the digital layer specifically — Bey's Temporary Autonomous Zone, and, in its own right, a Temporary Art-theatre Zone. **ZAT** — *Zona Artistico-teatrale Temporanea* — names one specific kind of event: the formal, publicized "Prova Generale," requiring an actual application to the Comune di Bosa. Ordinary, informal Provas are not ZATs and need no application — they run as often as needed, unannounced, the same as always. BosaTAZ, the digital layer, is developed and trialed during that ordinary-Prova stage, ahead of any formal ZAT.
...
**Current status:** La Prova, the physical performance, is well developed. BosaTAZ is not — hardware, software, and its integration with the performance layer all still need to be specified and built. This document describes the intended design; none of it is running yet.
...
| **Program tab** | **Live current-act state, posted by the admin, visible to anyone connected — no clock, no countdown** | **A printed theatre programme, made live** |
| **Participate tab** | **A static WhatsApp QR code and a plain what-to-bring checklist for next time — no live link, no explainer text** | **The poster's own QR-only, no-caption approach** |
| **Chain tab** | **Append-only history of the chain object's forms and trades, local to this Node, never exported or reset** | **A ship's log, kept for one object that never stops travelling** |
...
The presence board and relay run entirely on infrastructure already built for NCC (Nostr Comms for Communities), reconfigured to point at a relay running locally on the truck's server rather than any public Nostr relay. Voice was part of NCC's original scope but isn't carried into BosaTAZ — the relay here is text only, which is also a lighter load for the hardware to carry.

**The Program tab.** A fourth surface in the UI, sitting above the presence board rather than beside it — it needs no connection tier at all, not even the anonymous one, since it's pure information with nothing to witness. It borrows a completely different, already-legible convention than anything else in the app: a printed theatre programme, an object every visitor already knows how to read without being told. It lists the acts (L'Usato, L'Arte, La Promessa, Il Brindisi) with pre-written short explanations for each, and highlights whichever one is currently live.

State moves only when the admin, connected by USB, marks a transition — a single tap selecting the current act, nothing typed on the spot. This is posted to the local relay as a *replaceable* event rather than an accumulating note: unlike a ledger entry, which stays forever once posted, the Program tab only ever holds one current state, silently overwritten each time the admin advances it. Every connected phone's view updates the moment that happens, the same way pedestals appearing or the blackboard being mounted already signal a transition to anyone watching without a phone in hand.

No timer, no countdown, no estimated time remaining is shown anywhere in this feature — a live clock would reintroduce exactly the pressure to resolve the manifesto argues against, even for people who never look at it. If a sense of typical duration is ever wanted, the honest version pulls a soft range from the growing archive of past rehearsals rather than counting down a fixed one live.

**The Participate tab.** Exists for one visitor specifically: someone who came, watched, crossed no threshold all day, and wants a way back in beyond just returning and hoping to understand more next time. It answers "how do I get more involved" without ever explaining what anything meant — that's the Manifesto and FAQ's job, and neither lives on this air-gapped network anyway.

The one real technical constraint governs everything about how this is built: **BosaTAZ has no connection to the wider internet, ever, by design** — so a tappable WhatsApp link would simply fail while a phone is joined to the hotspot. The tab works around this by needing no live connection at all: it displays the WhatsApp group's QR code as a static image, the same object already painted on the truck itself, scannable whenever the visitor's own phone is back on ordinary signal — on the walk home, that evening, whenever. Showing a QR code costs nothing network-wise; only scanning one does, and that step happens entirely outside BosaTAZ's boundary.

A visitor can save that image to their phone with an ordinary long-press, the same as saving any photo from any website — no extra feature to build, since a static image already behaves this way in any browser. Saved to the gallery, it stays usable indefinitely: most phones can scan a QR code straight from a saved photo, not only from a live camera, so nobody needs to remember to reopen this page later. They can save it while still standing at the truck and act on it whenever signal returns, entirely on their own device, with BosaTAZ out of the loop by then.

Below the code, one line, no more: *bring something you no longer want (L'Usato), bring something worth a little more if you'd like to try bartering (the Gallery), think of a promise you could offer someone by name (the board).* A packing list, not an explainer — forward-looking, practical, and the only piece of text in the whole digital layer whose entire purpose is getting someone to come back better prepared.

**This is arguably the single most important reason the digital layer exists at all.** Every threshold in the physical day — Lo Spuntino, L'Usato, the Gallery, the board — has always been voluntary; nothing forces visibility before its moment. The code is that same rule applied one layer further, to understanding itself. It's also the one experience in the entire piece that's genuinely unwitnessed — every other moment is deliberately public, but reading the FAQ on a phone happens alone, at whatever pace someone chooses, seen by no one. And it needs nothing more than a shrug and a phone to complete: *I don't know what's going on, but apparently you can find out by scanning that QR code.* That sentence alone — no further action required — closes the loop for even the most passive, most novice aspetta-tore. They were never owed an explanation on the spot. They were only ever owed a way to find one, on their own terms, if they wanted it. The digital layer is what makes that true for absolutely everyone present, not just the ones who happened to already know who to ask.

**The Chain tab.** A different kind of record from everything else here — not a snapshot of one rehearsal, but the continuous history of one object across every rehearsal it's ever passed through. Each entry: what the object currently is, who traded for it, what was given up, and the date — recorded by the Remembrancer at the moment of each trade, the same way an ordinary promise is chalked, just kept in this separate, permanent thread rather than the day's ledger. Unlike the archive site, which exports out to BosaNode periodically, the Chain tab's history stays local and cumulative on the node itself — it's the one place BosaTAZ deliberately keeps growing rather than resetting between rehearsals, since the whole point is showing how far the chain has come. The closing entry each day the object was traded is the Remembrancer's custody line — see roles/il-custode.md and the Remembrancer's own role description for the exact form this takes — naming whoever holds it until the next Prova.
...
7. **No overview, no landing page.** No tab exists purely to explain what Slow Theatre is or introduce who's involved — every surface is oriented toward doing something (seeing who's present, seeing the live state, getting what's needed to join in) rather than being told something first. This wasn't planned as a rule from the start; the Program tab and the Participate tab each arrived at it independently, for their own local reasons, and only look like one consistent stance in hindsight. Explanation exists, but only downstream and opt-in — reached through the Participate tab's QR code, never offered as the entry point itself. The digital layer inherited this from the physical one without anyone deciding it should: La Prova never opens with a welcome speech either.
...
- The growth sequence is now: (1) ordinary Prova, MVP physical version, run as often as needed, no application required; (2) BosaTAZ trialed and integrated during that ongoing ordinary-Prova stage; (3) once both are working, a proposal is developed for a formal ZAT — a single, publicized one-day event in Bosa, the "Prova Generale," requiring an actual comune application; (4) only after a ZAT has run successfully does a PPS (Piano Straordinario di Solidarietà e Scambio di Prossimità) proposal become makeable — a later, separate pitch for permanent suolo pubblico rights, made in collaboration with other local cultural groups rather than alone.
