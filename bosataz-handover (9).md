...
6. **Chain tab** — append-only, local, and never reset — the one
   ...
   when. Closing entry on any day it was traded is the custody
   line, naming whoever holds it until the next Prova (see
   roles/il-custode.md). Does not sync to BosaNode the same way the
   archive site does — this history is local to the Node by design.
